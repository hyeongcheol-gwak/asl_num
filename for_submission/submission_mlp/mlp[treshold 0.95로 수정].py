...
def main():
    ...
    extracted_frames = extract_frames_from_video(video_file, threshold=0.95)
    ...
    for idx, frame in enumerate(extracted_frames):
        ...
        if results.multi_hand_landmarks:
            ...
            # 가장 많이 나온 라벨 선택
            final_prediction = Counter(votes).most_common(1)[0][0] + 1
            detected_gestures.append(final_prediction)
            print(
                f"이미지[{idx:03d}] | 예측 결과: {final_prediction} | 투표 분포: {Counter(votes)}"
            )
            # -----------------------
        else:
